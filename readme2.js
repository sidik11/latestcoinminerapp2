    ...
    // Login functionality with member code verification
    loginBtn.addEventListener('click', async () => {
      ...
      try {
        // Check if member code exists in either path
        const diamondMemberRef = ref(db, 'slidediamondmember');
        const slideMemberRef = ref(db, 'slidemember');
        
        const [diamondSnapshot, slideSnapshot] = await Promise.all([
          get(diamondMemberRef),
          get(slideMemberRef)
        ]);
        
        let memberCodeExists = false;
        
        // Check diamond members
        if (diamondSnapshot.exists()) {
          const diamondMembers = diamondSnapshot.val();
          for (const userId in diamondMembers) {
            if (diamondMembers[userId].memberCode === memberCode) {
              memberCodeExists = true;
              break;
            }
          }
        }
        
        // Check regular members if not found in diamond
        if (!memberCodeExists && slideSnapshot.exists()) {
          const slideMembers = slideSnapshot.val();
          for (const userId in slideMembers) {
            if (slideMembers[userId].memberCode === memberCode) {
              memberCodeExists = true;
              break;
            }
          }
        ...
        if (!memberCodeExists) {
          alert("❌ You are not a SLIDE member. Please register first to get a member code.");
    ...
    async function transferCoins() {
      ...
      try {
        ...
        // Save sessionCoins to history BEFORE resetting
        const coinsToTransfer = sessionCoins;
        ...
        // Now reset session coins
        sessionCoins = 0;
        sessionCoinsEl.textContent = sessionCoins;
        localStorage.setItem('sessionCoins', sessionCoins);

        // Show success message
        miningStatsEl.textContent = `Success! ${coinsToTransfer} coins transferred to your account.`;
